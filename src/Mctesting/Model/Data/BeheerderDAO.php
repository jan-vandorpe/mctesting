...
class BeheerderDAO {
    ...
    public static function insert($user, $password, $timestamp) {
        ...
        $stmt = $db->prepare($sql);
        //test if statement can be executed
        if ($stmt->execute(array(':email' => $user->getEmail()))) {
            $recordset = $stmt->fetchAll();
            if (empty($recordset)) {
                ...
                if ($stmt->execute(array(':RRNr' => $user->getRRnr(), ':email' => $user->getEmail(), ':firstName' => $user->getFirstName(), ':lastName' => $user->getLastName(), ':wachtwoord' => $password, ':group' => $user->getGroup(), ':timestamp' => $timestamp))) {
                    ...
                    $error = $stmt->errorInfo();
                    if ($error[1] == 1062) {
                        throw new ApplicationException('De beheerder (' . $user->getRRnr() . ') bestaat al. <br> Gelieve een ander rijksregisternummer in te vullen.');
                    } else {
