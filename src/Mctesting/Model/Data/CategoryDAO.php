...
class CategoryDAO
{
    ...
    public static function selectAllExceptEmpty()
    {
        ...
        $db = new \PDO(DB_DSN, DB_USER, DB_PASS);
        //prepare sql statement
        $sql = 'SELECT categorie.catid as catid, categorie.catnaam as catnaam, categorie.actief as actief FROM categorie inner join (subcategorie inner join vraag on subcategorie.subcatid = vraag.subcatid) on subcategorie.catid = categorie.catid WHERE subcategorie.actief = 1 GROUP BY catid ORDER BY catnaam';
        $stmt = $db->prepare($sql);
        //test if statement can be executed
        if ($stmt->execute())
        {
